api.controller = function($scope, $window, $timeout, $sce, $location, $sanitize) {
    ...
    function processMessage(msg) {
        ...
        let parsedPayload = (msg.type === BOT_TYPE) ? parseJson(msg.text) : null;
        return {
            type: msg.type,
    ...
    c.handleBotAction = function(action, payload) {
        ...
        if (action === 'confirm') {
            c.sendMessage(c.data.i18n.auto_reply_confirm, { force_tool: 'create_incident' });
        } else if (action === 'cancel') {
            c.sendMessage(c.data.i18n.auto_reply_cancel);
        }
    };
    ...
    c.sendMessage = function(overrideQuery, options) {
        ...
        let requestPayload = {
            action: 'send_message_with_reply',
        ...
        c.server.get(requestPayload).then(function(response) {
            ...
            let payload = parseJson(rawResponse);
    ...
    c.startNewChat = function() {
        ...
        c.isLoading = true;
        c.server.get({ action: 'create_new_conversation', name: c.data.i18n.default_new_chat_name }).then(function(response) {
        ...
        $timeout(function() {
            c.showMessageOverlay = {
                ...
                confirmAction: function() {
                    ...
                    c.server.get({ action: 'disable_conversation', sys_id: sysId }).then(function() {
    ...
    function loadMessages(conversationId) {
        ...
        c.isLoading = true;
        c.messages = [];
        c.server.get({ action: 'fetch_messages', active_sys_id: conversationId }).then(function(response) {
            $timeout(function() {
    ...
    c.finishRename = function() {
        ...
        c.isRenaming = false;
        c.server.get({ action: 'rename_conversation', sys_id: c.activeConversation.id, name: c.activeConversation.name });
    };
